...
enum class DeviceHardware {
	none = 0,
	pin = 1, //!< A digital pin, either input or output
	onewireTemp = 2,	//<! A onewire temperature sensor
#if BREWPI_DS2413
	onewire2413 = 3	//<! A onewire 2-channel PIO input or output.
#endif
};


/**
 * \brief Configuration of a Device
 */
struct DeviceConfig {
   ...
   *
   * Hardware level configuration data.  Describes how the to reach the
   ...
   */
    ...
    /**
     * \brief Sensor calibration/PIO
     * The pio and sensor calibration are never needed at the same time so they
     * are a union.  To ensure the eeprom format is stable when
     * including/excluding DS2413 support, ensure all fields are the same size.
     */
		union {
#if BREWPI_DS2413
			uint8_t pio;	//!< For DS2413 (DeviceHardware::onewire2413) : the pio number (0,1)
#endif
      /**
       * \brief Temp sensor calibration factor
       *
       * For temp sensors (DeviceHardware::onewireTemp), calibration adjustment
       * to add to sensor readings this is intentionally chosen to match the
       * raw value precision returned by the DS18B20 sensors
       */
			int8_t calibration;
		};
	...
	bool reserved2; //!< Extra space so that additional fields can be added without breaking layout
};
/** @} */
