...
public:
   ...
   */
  PiStream(StreamType &stream)
      : upstream(stream), stream{stream, Config::PiLink::printBufferSize()} {};
   ...
   */
  int readPersistent(int timeout = 10) {
    uint8_t retries = 0;
    while (available() == 0) {
   ...
   */
  void print(char out) { stream.print(out); };
   ...
   */
  void print(String out) { print(out.c_str()); };
   ...
   * message \param doc - Reference to JsonDocument that should be sent
   */
  void sendJsonMessage(char prefix, JsonDocument &doc) {
    if (prefix) {
      print(prefix);
      print(':');
    }
   ...
   */
  void receiveJsonMessage(JsonDocument &doc) { deserializeJson(doc, stream); }
  ...
  typename std::enable_if<!std::is_same<U, HardwareSerial>::value &&
                          !std::is_same<U, WiFiClient>::value>::type
  init(){};
   ...
   */
  template <typename U = StreamType>
  typename std::enable_if<std::is_same<U, WiFiClient>::value>::type init() {
    ::initWifiServer();
  };
   ...
   */
  template <typename U = StreamType>
  typename std::enable_if<std::is_same<U, HardwareSerial>::value>::type init() {
...
private:
  ...
  /**
   * \brief Pointer to the wrapped StreamType object
   *
  ...
  /**
   * \brief Pointer to the Stream used for communications
   */
   ...
   */
  template <typename U = StreamType>
  typename std::enable_if<!std::is_same<U, WiFiClient>::value, bool>::type
  _bool() {
    return bool(stream);
  };
...
template <typename T>
char PiStream<T>::printfBuff[Config::PiLink::printfBufferSize];
